import AreaChart from '../../patient/components/AreaChart';
import { options } from '../../patient/components/DoughnutChart';
...
export default function Home() {
    const { data, loading, error } = useFetch<Report>('/report/getLatest', 'doctor');
    const { data: doctorData } = useFetch<Doctor>('/doctor/get', 'doctor');
    ...
    return (
        <div>
            <div className='px-5'>
                <div className="bg-white shadow-lg rounded-lg p-6 w-full mt-5">
                    <h2 className="text-2xl font-medium text-gray-900 -mb-3 ">
                        {loading || error ?
                            (<div className="w-16 h-5 bg-gray-400 rounded-sm animate-pulse">&nbsp;</div>)
                            :
                            (<div>
                                <div> Welcome Back, Dr {doctorData && doctorData.name} </div>
                                <div className="text-sm text-gray-600 mt-3">
                                    <div className='italic'>
                                        <p>Email: {doctorData && doctorData.email}</p>
                                        <p>Speciality: {doctorData && doctorData.speciality}</p>
                                    </div>
                                </div>
                            </div>
                            )
                    ...
                    <div className="flex items-center justify-end text-sm text-gray-600">
                        <div className='-mt-10'>
                            <svg fill="#000000" width="80px" height="80px" viewBox="-1.5 0 24 24" xmlns="http://www.w3.org/2000/svg">
                                ...
                                <path d="m16.225 17.965v-.001c0-.372-.301-.673-.673-.673s-.673.301-.673.673.301.673.673.673c.371 0 .672-.301.673-.672z" /></svg>
                        </div>
                    </div>
                </div>
            </div>
            {loading || error ?
                (
                    <div className="stats  flex items-center my-5 mx-5">
                ...
                :
                (
                    <div className="stats shadow flex items-center my-5 mx-5">
                        <div className="stat place-items-center">
                            <div className="stat-title">Daily Number of Records</div>
                            <div className="stat-value"> {data && data.noOfCheckIns} </div>
                        </div>

                        <div className="stat place-items-center">
                            <div className="stat-title">Number Of Assigned Patients</div>
                            <div className="stat-value">{data && data.noOfPatients}</div>
                        </div>

                        <div className="stat place-items-center">
                            <div className="stat-title">New Registers</div>
                            <div className="stat-value">{data && data.noOfNewRegisters}</div>
            ...
            <div className='flex flex-col'>
                <div className="flex flex-row justify-around mb-5">
                    <div className='w-2/4 border-4 border-white rounded-lg bg-white mx-5 h-1/4'>
                        <div className='flex flex-row relative'>
                            <div className='flex items-center h-16 w-full pl-5 bg-white'>
                                Record & Percentage Statistics
                            </div>
                        </div>
                        ...
                        {loading || error ?
                            (<div className='flex justify-center'>
                                <div className='h-fit w-80'>
                                    <div className="w-96 h-80 my-5 -ml-10 bg-gray-400 rounded-s animate-pulse">&nbsp;</div>
                                </div>
                            </div>)
                            :
                                        ...
                                        data={
                                            {
                                                ...
                                                datasets: [
                                                    {
                                                        label: '# of Patients',
                                                        data: [data?.noOfCheckedRecords, data?.noOfUncheckedRecords],
                                                        backgroundColor: [
                                                            'rgba(75, 192, 192, 0.2)',
                                                            'rgba(255, 99, 132, 0.2)',
                                                        ],
                                    ...
                                    <Doughnut options={options}
                                        data={{
                                            ...
                                            datasets: [
                                                {
                                                    label: '# of Patients',
                                                    data: [data?.noOfMalePatients, data?.noOfFemalePatients],
                                                    backgroundColor: [
                                                        'rgba(54, 162, 235, 0.2)',
                    ...
                    <div className='border-4 border-white rounded-lg w-2/4 bg-white mx-5 h-1/4'>
                        <div className='flex flex-row relative'>
                            <div className='flex items-center h-16 w-full pl-5 bg-white-100'>
                                Graph of Number of Record Per Month
                            </div>
                        </div>
                        {loading || error ?
                            (<div className='flex justify-center'>
                            ...
                            :
                            (<div>
                                <div style={{ height: "18rem", width: "32rem" }}>
                                    {data && <AreaChart data={data.noOfRecordsPerMonth} />}                            </div>
                            </div>)
                        }
                    </div>
                </div>
